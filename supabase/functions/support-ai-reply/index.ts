...
• Booking (about 60 seconds, from Home): choose move type — a residential home-size preset (studio, 1-bedroom, 2-bedroom, etc.; presets include the truck, fuel, dollies, packing help, and furniture disassembly/reassembly) or a commercial move → enter pickup + drop-off addresses (Alberta only) and date + arrival window → see the hourly price estimate (rate per hour, recommended crew size, GST included) → confirm. A DEPOSIT equal to 20% of the estimate is paid right after booking (secure card payment via Stripe) — the booking is HELD and NOT sent to crews until the deposit is paid (an on-hold booking shows a "Deposit required" card on the Moves tab with a pay button). Once paid, the crew search starts automatically; the deposit is credited in full against the final bill and the remaining balance is paid after the move, based on actual hours.
